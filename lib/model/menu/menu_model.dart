...
class MenuModel {
  ...
  num? total;
  num? totalSale;
  double? updataPrice1 = 0.0;
  ItemCurr? itemCurreny;
  double? discountDollor;
  double? showDiscountCurr; // use when discount with currency
  ...
  factory MenuModel.fromMap(Map<String, dynamic> data) => MenuModel(
    ...
    updataPrice1: (data["UPDATE_PRICE"] as num?)?.toDouble(),
    discountDollor: (data["DISCOUNT_DOLLAR"] as num?)?.toDouble(),
    discountPercent: (data["DISCOUNT_PERCENT"] as num?)?.toDouble(),
    itemCurreny: data["ITEM_CURR"] == null ? null : ItemCurr.fromJson(data["ITEM_CURR"]),
  );
  ...
  Map<String, dynamic> toMap() => {
    ...
    'DISCOUNT_DOLLAR': discountDollor,
    "ITEM_CURR": itemCurreny?.toJson(),
  };
  ...
  static List<MenuModel> fromList(List<dynamic> list) {
    return list.map((item) => MenuModel.fromMap(item as Map<String, dynamic>)).toList();
  }
